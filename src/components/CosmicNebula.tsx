...
export default function CosmicNebula({
...
}: CosmicNebulaProps): React.ReactElement {
  const mountRef = useRef<HTMLDivElement | null>(null);
  const webglRef = useRef<CosmicNebulaWebGL | null>(null);
  ...
  useEffect(() => {
    ...
    function makePaletteTexture(stops: string[]): THREE.DataTexture {
      ...
      // Upsample to a smooth 256-wide gradient for richer color transitions
      const w = 256;
      const data = new Uint8Array(w * 4);
      for (let i = 0; i < w; i++) {
        const t = i / (w - 1);
        const seg = t * (arr.length - 1);
        const idx = Math.min(Math.floor(seg), arr.length - 2);
        const frac = seg - idx;
        const cA = new THREE.Color(arr[idx]);
        const cB = new THREE.Color(arr[idx + 1]);
        cA.lerp(cB, frac);
        data[i * 4 + 0] = Math.round(cA.r * 255);
...
void main(){
  ...
  gl_FragColor = vec4(newv, 0.0, 0.0);
    ...
    class Advection extends ShaderPass {
      ...
      update(...args: any[]) {
        const { dt: _dt, isBounce: _bounce, BFECC: _bfecc } = (args[0] || {}) as any;
        if (!this.uniforms) return;
        if (typeof _dt === 'number') this.uniforms.dt.value = _dt;
        if (typeof _bounce === 'boolean') this.line.visible = _bounce;
    ...
    class Viscous extends ShaderPass {
      ...
      update(...args: any[]) {
        const { viscous: _v, iterations, dt: _dt } = (args[0] || {}) as any;
        if (!this.uniforms) return;
        let fbo_in: any, fbo_out: any;
        if (typeof _v === 'number') this.uniforms.v.value = _v;
    ...
    class Pressure extends ShaderPass {
      ...
      update(...args: any[]) {
        const { vel, pressure: p } = (args[0] || {}) as any;
    ...
    // ── WebGL manager ─────────────────────────────────────────────
    class WebGLManager implements CosmicNebulaWebGL {
      props: any;
      output!: Output;
